		...
		if (isEmpty(user)) {
			...
			Account.find({ 'email': req.body.email }, function (err, user) {
				...
				if (isEmpty(user)) {
					...
					var new_account = new newAccount(req.body.firstname, req.body.lastname, req.body.username, hashedPassword, req.body.email, null);
			...
			bcrypt.compare(req.body.password, user[0].password, function (err, result) {
				...
				if (result) {
					...
					if (user[0].status == 'verified') {
						...
						return res.status(403).send({ auth: false, message: 'Account not verified.' });
					}
